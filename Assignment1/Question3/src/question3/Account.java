...
public class Account {
    ...
    public String getName() { // Gets the name of account
        return name;
    }

    public Account(String name, double balance) { // Creates the Account with name and balance inputs  
        this.name = name; // Sets name as input
        setBalance(balance); // Sets balance as input
    }
//
    ...
    {
        if (deposit < 1) // Deposit cannot be less then $1
        {
            System.out.println("Error! Invalid Deposit!");
            this.balance = balance; // Prints error and will still have the same balance
        }
        else
        {
            this.balance = deposit + balance; // Otherwise balance is upadted with deposit added
        }
        return balance; // Returns balance
    }
    public double withdraw(double withdraw) { // Withdraws money from account
        if (withdraw < 1) // Withdraw cannot be less then $1
        {
